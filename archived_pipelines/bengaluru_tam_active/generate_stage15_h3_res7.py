...
STAGE1_DIR = PROCESSED_DATA_DIR / "Final stage 1 data"
STAGE1_LOCALITIES_PATH = STAGE1_DIR / "stage1_locality_features.json"
STAGE1_H3_CELLS_PATH = STAGE1_DIR / "h3_heatmap_cells.geojson"
RAW_LOCALITIES_PATH = RAW_DATA_DIR / "bangalore_localities_enriched.json"

OUTPUT_JSON = PROCESSED_DATA_DIR / "stage1_5_hex7_features.json"
OUTPUT_CSV = PROCESSED_DATA_DIR / "stage1_5_hex7_features_flat.csv"
OUTPUT_GEOJSON = PROCESSED_DATA_DIR / "stage1_5_hex7_cells.geojson"
AUDIT_PATH = AUDIT_DIR / "stage1_5_hex7_audit.json"
...
def in_bengaluru_bounds(lat, lon):
    lat = clean_numeric(lat)
    lon = clean_numeric(lon)
    if lat is None or lon is None:
        return False
    return (
        BENGALURU_METRO_BOUNDS["min_lat"] <= lat <= BENGALURU_METRO_BOUNDS["max_lat"]
        and BENGALURU_METRO_BOUNDS["min_lon"] <= lon <= BENGALURU_METRO_BOUNDS["max_lon"]
    )
...
def add_locality_to_group(groups, stage_record, raw_lookup, missing_raw_ids):
    ...
    within_bounds = in_bengaluru_bounds(locality.get("lat"), locality.get("lon"))
    ...
    group["localities"].append(
        {
            ...
            "rental_yield_pct": market.get("rental_yield_pct"),
            "yearly_appreciation_pct": market.get("yearly_appreciation_pct"),
            "within_bengaluru_bounds": within_bounds,
            "quality_flags": [] if within_bounds else ["outside_bengaluru_metro_bounds"],
        }
    )
    return True
...
def finalize_group(hex_id, group, h3_8_rollups):
    ...
    return {
        ...
        "quality": {
            ...
            "within_bengaluru_bounds_locality_count": len(localities)
            ...
            "out_of_bounds_examples": [
                {
                    ...
                    "localities": [
                        ...
                        if not locality.get("within_bengaluru_bounds")
                    ],
                }
                for record in records
                if "contains_out_of_bounds_localities" in record.get("quality", {}).get("flags", [])
