...
STAGE2_MASTER = PROCESSED_DIR / "stage2_hex7_affluence_master.json"
...
OUTPUT_JSON = FINAL_DIR / "bangalore_hex7_affluent_family_intelligence_master.json"
OUTPUT_CSV = FINAL_DIR / "bangalore_hex7_affluent_family_intelligence_flat.csv"
OUTPUT_GEOJSON = FINAL_DIR / "bangalore_hex7_affluent_family_intelligence.geojson"
OUTPUT_KML = FINAL_MAP_DIR / "bangalore_hex7_affluent_family_intelligence.kml"
OUTPUT_README = FINAL_DIR / "README_bangalore_hex7_affluent_family_intelligence.md"
AUDIT_JSON = AUDIT_DIR / "final_hex_intelligence_audit.json"
METHODOLOGY_MD = AUDIT_DIR / "final_hex_intelligence_methodology.md"

SCHOOL_AGE_FAMILY_RATE = 0.38
CHILDREN_PER_SCHOOL_AGE_FAMILY = 1.25

COUNTABLE_60L_PLUS_BANDS = {"60L-1.5Cr", "1.5Cr+"}
...
def estimate_conservative_50l_plus_tam(income_band_family_tam):
    ...
    for band, values in safe_dict(income_band_family_tam).items():
        if band in COUNTABLE_60L_PLUS_BANDS:
            total += num(safe_dict(values).get("direct"))
    return total


def estimate_40l_plus_linear_tam(income_band_family_tam):
    # Target TAM = Premium onwards (AHI >= 15L) = 100% of 1.5Cr+, 60L-1.5Cr, 30L-60L, and 15L-30L
    ib = safe_dict(income_band_family_tam)
    return (
        num(ib.get("1.5Cr+", {}).get("direct"))
        + num(ib.get("60L-1.5Cr", {}).get("direct"))
        + num(ib.get("30L-60L", {}).get("direct"))
        + num(ib.get("15L-30L", {}).get("direct"))
    )
...
def final_record(stage2_record):
    ...
    countable_family_tam = estimated_40l_plus_linear if num(tam.get("countable_direct_family_tam")) > 0 else 0.0
    ...
    school_access_score = num(scores.get("school_access_score", 0.0))
    countable_wealthy_school_children = countable_school_age_children * school_access_score
...
def kml_description(record):
    ...
    <div style="font-family:Arial,sans-serif;max-width:800px;color:#111827;">
      ...
      {section("TAM Derivation", mini_table(["Metric", "Value"], [
        ...
        ["Conservative 60L+ family TAM", fmt_number(tam["conservative_50l_plus_family_tam"])],
        ["Conservative 60L+ share of direct TAM", fmt_number(100 * tam["conservative_50l_plus_share_of_direct_tam"], 1) + "%"],
        ["Estimated 40L+ AHI TAM", fmt_number(tam["estimated_40l_plus_family_tam_linear_25_50_split"])],
        ["Estimated 40L+ AHI share", fmt_number(100 * tam["estimated_40l_plus_share_linear_25_50_split"], 1) + "%"],
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>Bangalore Hex-7 Affluent Family Intelligence</name>
    {''.join(styles)}
...
def write_methodology(records, audit):
    text = f"""# Final Hex-7 Affluent Family Intelligence Methodology
...
35 km/h travel-time conversion from Stage 2. Families in Bengaluru often use buses,
...
`conservative_50l_plus_family_tam` counts only direct TAM in the conservative 60L+ AHI bands:

- 60L-1.5Cr
- 1.5Cr+

The `30L-60L` band is not fully counted in this conservative measure because it crosses the 40 LPA threshold. 

Across the current final output, the conservative 60L+ direct AHI TAM share is
{audit['income']['conservative_50l_plus_share_pct']}%. If the 30L-60L band is split
linearly and the 40L-60L slice is counted, the estimated 40L+ direct AHI TAM share is
...
def write_readme(audit):
    text = f"""# Bangalore Hex-7 Affluent Family Intelligence
...
- Countable school-age children: {audit['tam_totals']['countable_school_age_children']}
- Countable wealthy-school children: {audit['tam_totals']['countable_wealthy_school_children']}
- Conservative 60L+ direct TAM share: {audit['income']['conservative_50l_plus_share_pct']}%
- Estimated 40L+ direct AHI TAM share: {audit['income']['estimated_40l_plus_share_linear_25_50_split_pct']}%
...
def main():
    ...
    payload = {
        "metadata": {
            "title": "Bangalore Hex-7 Affluent Family Intelligence",
