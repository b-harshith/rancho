...
async def scrape_practo_hospitals():
    ...
    async with async_playwright() as p:
        ...
        while True:
            url = f'https://www.practo.com/search/hospitals?results_type=hospital&q=%5B%7B%22word%22%3A%22hospital%22%2C%22autocompleted%22%3Atrue%2C%22category%22%3A%22type%22%7D%5D&city=bangalore&page={page_num}'
    ...
    # Save as JSON
    json_path = "data/practo_hospitals_bangalore.json"
    with open(json_path, "w") as jf:
        json.dump(hospitals_list, jf, indent=2)
    print(f"\nSaved {len(hospitals_list)} hospitals to JSON: {json_path}")
    
    # Save as JSONL
    jsonl_path = "data/practo_hospitals_bangalore.jsonl"
