...
def main():
    ...
    print(f"Global Q3 threshold (75th percentile of max purchase price): INR {q3:,.2f} ({q3/10000000:.2f} Crore)")
    
    q4_societies = [s for s in valid_societies if s["rei"]["sale"]["price"]["max"] >= q3]
    # Sort descending by max price
    q4_societies.sort(key=lambda x: x["rei"]["sale"]["price"]["max"], reverse=True)
    
    # Categorize based on max purchase price:
    # - Ultra Luxury: >= 4.5 Cr (45,000,000)
    # - Super Luxury: 3.0 Cr - 4.5 Cr (30,000,000 - 44,999,999)
    # - Luxury: 2.6 Cr - 3.0 Cr (26,000,000 - 29,999,999)
    # - Premium: 2.1 Cr - 2.6 Cr (21,000,000 - 25,999,999)
    categorized = []
    for s in q4_societies:
    ...
    # Write to JSON
    json_output_path = "data/q4_categorized_societies_bangalore.json"
    json_data = []
    ...
    artifact_path = "/Users/malleswararao/.gemini/antigravity-ide/brain/5ad9d68c-7c50-4a98-8496-16e26f027f49/q4_categorized_societies.md"
    
    # Calculate counts and units
    counts = {"Ultra Luxury": 0, "Super Luxury": 0, "Luxury": 0, "Premium": 0}
    units_counts = {"Ultra Luxury": 0, "Super Luxury": 0, "Luxury": 0, "Premium": 0}
    ...
    with open(artifact_path, "w", encoding="utf-8") as out:
        out.write(f"# Categorized Q4 Premium & Luxury Societies (Bangalore)\n\n")
        out.write(f"This document breaks down the **{len(json_data)}** Q4 residential societies (top 25% by maximum listing price, threshold: **>= INR {q3:,.2f}** / **2.1 Crore**) into four sub-quartiles with corresponding luxury labels:\n\n")
        ...
        out.write("|---|---|---|---|\n")
        for cat in ["Ultra Luxury", "Super Luxury", "Luxury", "Premium"]:
            rng = ">= 4.5 Crore" if cat == "Ultra Luxury" else "3.0 - 4.5 Crore" if cat == "Super Luxury" else "2.6 - 3.0 Crore" if cat == "Luxury" else "2.1 - 2.6 Crore"
    ...
    print(f"Society analysis completed: {len(json_data)} premium societies written to JSON and Markdown.")
