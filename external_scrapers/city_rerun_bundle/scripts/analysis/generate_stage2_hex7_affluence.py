...
MAPS_DIR = Path("maps") / CITY_SLUG / "h3"

STAGE15_HEX_PATH = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage1_5_hex7_features.json"
...
SEZ_KML_PATH = Path(os.environ.get("SEZ_KML_PATH", str(STAGE2_DIR / "sez_office_zones.kml")))
OVERTURE_BUILDINGS_PATH = Path(
    os.environ.get(
        "OVERTURE_BUILDINGS_PATH",
        str(DATA_DIR / "overture" / f"{CITY_SLUG}_buildings.geojson"),
    )
)

OUTPUT_JSON = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage2_hex7_affluence_master.json"
OUTPUT_CSV = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage2_hex7_affluence_master_flat.csv"
OUTPUT_GEOJSON = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage2_hex7_affluence_master.geojson"
OUTPUT_LINKS = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage2_poi_hex_links.jsonl"
ROUTING_CACHE_PATH = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage2_routing_cache.json"
HABITABILITY_CACHE_PATH = PROCESSED_DATA_DIR / f"{CITY_SLUG}_stage2_hex7_habitability_from_overture.json"
OUTPUT_KML = MAPS_DIR / f"{CITY_SLUG}_stage2_hex7_affluence_master.kml"
AUDIT_JSON = AUDIT_DIR / f"{CITY_SLUG}_stage2_hex7_affluence_audit.json"
METHODOLOGY_MD = AUDIT_DIR / f"{CITY_SLUG}_stage2_hex7_affluence_methodology.md"
...
CITY_MAP_CENTER = json.loads(
    os.environ.get(
        "CITY_MAP_CENTER_JSON",
        json.dumps([12.9716, 77.5946]),
    )
)

OSRM_URL = os.environ.get("OSRM_URL", "http://localhost:5001").rstrip("/")
AVG_SPEED_KMPH = 35.0
MATRIX_CHUNK_SIZE = 75
...
SOCIETY_INCOME_BANDS = {
    "Ultra Luxury": "5Cr+",
    "Super Luxury": "2Cr-5Cr",
    "Luxury": "1Cr-2Cr",
    "Premium": "50L-1Cr",
    "Aspirational Premium": "25L-50L",
}
...
class OSRMClient:
    ...
    def save(self):
        self.cache["metadata"] = {
            "routing_method": "osrm_local",
            "osrm_url": self.base_url,
    ...
    def validate(self):
        try:
            response = requests.get(
                f"{self.base_url}/nearest/v1/driving/{CITY_MAP_CENTER[1]},{CITY_MAP_CENTER[0]}",
                timeout=5,
            )
            ...
            raise RuntimeError(
                "Local OSRM is required but is not reachable. "
                f"Set OSRM_URL or start osrm-routed at {self.base_url}. "
                "No proxy routing fallback is enabled."
            ) from exc
        except RuntimeError as exc:
            raise RuntimeError(
                "Local OSRM responded, but the nearest endpoint did not return code=Ok. "
                f"Check that the loaded OSM extract covers {CITY_NAME}."
            ) from exc
    ...
    def matrix_distances(self, source, targets):
        ...
        for offset in range(0, len(pending), MATRIX_CHUNK_SIZE):
            chunk = pending[offset : offset + MATRIX_CHUNK_SIZE]
            chunk_indices = pending_indices[offset : offset + MATRIX_CHUNK_SIZE]
            coordinates = [source, *chunk]
            coordinate_text = ";".join(
                f"{point['lon']:.6f},{point['lat']:.6f}" for point in coordinates
            )
            destinations = ";".join(str(index) for index in range(1, len(coordinates)))
            try:
                response = requests.get(
                    f"{self.base_url}/table/v1/driving/{coordinate_text}",
                    params={
                        "sources": "0",
                ...
                if data.get("code") != "Ok":
                    raise RuntimeError(data)
                matrix_rows = data.get("distances") or []
                matrix = matrix_rows[0] if matrix_rows else []
            except requests.RequestException:
                self.failures["matrix_request_failed"] += len(chunk)
                matrix = []
            except RuntimeError:
                self.failures["matrix_response_not_ok"] += len(chunk)
                matrix = []

            if len(matrix) != len(chunk):
                for local_index, target in enumerate(chunk):
                    result = self.route_distance(source, target)
                    if result:
                        results[chunk_indices[local_index]] = result
                continue

            self.request_count += 1
            for local_index, item in enumerate(matrix):
                target = chunk[local_index]
                key = route_key(source, target)
                distance_meters = clean_float(item)
                distance = distance_meters / 1000.0 if distance_meters is not None else None
                if distance is None:
                    self.failures["matrix_distance_missing"] += 1
                    continue
                result = {
                    "distance_km": distance,
                    "travel_time_min_at_35_kmph": distance / AVG_SPEED_KMPH * 60.0,
                    "method": "osrm_local",
                }
    ...
    def route_distance(self, source, target):
        ...
        if cached:
            ...
            return cached
        coordinates = (
            f"{source['lon']:.6f},{source['lat']:.6f};"
            f"{target['lon']:.6f},{target['lat']:.6f}"
        )
        try:
            response = requests.get(
            ...
            data = response.json()
            if data.get("code") != "Ok":
                self.failures["route_response_not_ok"] += 1
                return None
            routes = data.get("routes") or []
            summary = safe_dict(routes[0]) if routes else {}
            distance_meters = clean_float(summary.get("distance"))
            distance = distance_meters / 1000.0 if distance_meters is not None else None
            if distance is None:
                self.failures["route_distance_missing"] += 1
                return None
            self.request_count += 1
            result = {
                "distance_km": distance,
            ...
            self.failures["route_request_failed"] += 1
            return None
...
def apply_normalized_scores(raw_by_hex):
    ...
    for hex_id in hex_ids:
        ...
        school_score = (
            0.40 * normalized["premium_school_travel_access"][hex_id]
            + 0.25 * normalized["annual_fee_travel_weighted"][hex_id]
            + 0.20 * normalized["student_tam_travel_weighted"][hex_id]
            + 0.15 * normalized["top_school_count_access"][hex_id]
        )
        hospital_score = (
        ...
        evidence_confidence = (
            ...
            * max(
            ...
            + 0.20 * min(1.0, raw["schools_nearby_count"] / 3.0)
        ...
        raw["component_scores"] = {
            ...
            "school_score": round(residential_school_fit_score, 6),
            "school_access_score": round(school_score, 6),
            "residential_school_fit_score": round(residential_school_fit_score, 6),
...
def build_master_records(raw_by_hex):
    ...
    for rank, hex_id in enumerate(sorted_hexes, start=1):
        ...
        school_age_families = direct_family_tam * SCHOOL_AGE_FAMILY_RATE
        school_age_children = school_age_families * CHILDREN_PER_SCHOOL_AGE_FAMILY
        school_access_score = raw["component_scores"]["school_access_score"]
        wealthy_school_children = (
            school_age_children * school_access_score
        )
        habitability = raw["habitability"]
        ...
        countable_school_age_families = school_age_families if habitable_for_tam else 0.0
        countable_school_age_children = school_age_children if habitable_for_tam else 0.0
        countable_wealthy_school_children = wealthy_school_children if habitable_for_tam else 0.0
        ...
        if raw["societies_nearby_count"] == 0 and raw["schools_nearby_count"] == 0:
            quality_flags.append("low_residential_school_poi_evidence")
        ...
        record = {
            ...
            "tam": {
                ...
                "estimated_wealthy_school_families": round(
                    school_age_families * school_access_score, 2
                ),
...
This pipeline uses local OSRM routing only. If OSRM is unavailable, the script exits without
creating routed Stage 2 outputs.

## Final score

`base_affluence_score = 100 * (0.45 society + 0.14 residential_school_fit + 0.15 hospital + 0.21 market + 0.05 SEZ)`

The society component now separates direct/nearby residential evidence from cluster context:
...
The school component is bus-aware. Schools can remain eligible for longer routes, especially premium schools,
but only the top `{SCHOOL_SCORE_TOP_N}` school utilities affect scoring. The first `{SCHOOL_BUS_FREE_MIN}` minutes
do not decay, then school access decays by category-specific tau:

`school_bus_decay = exp(-max(0, travel_time_min - {SCHOOL_BUS_FREE_MIN}) / tau)`

The ranking uses `residential_school_fit_score`, not raw school access:

`residential_school_fit_score = school_access_score * max(society_score, 0.75 * society_cluster_score, 0.60 * market_score)`

This prevents schools alone from creating residential wealth evidence.
...
OSRM endpoint: `{OSRM_URL}`

Travel time is computed from OSRM route distance using `{AVG_SPEED_KMPH}` km/h:
...
School-age and wealthy-school estimates are modeled estimates:

`school_age_families = direct_family_tam * {SCHOOL_AGE_FAMILY_RATE}`

`school_age_children = school_age_families * {CHILDREN_PER_SCHOOL_AGE_FAMILY}`

`wealthy_school_children = school_age_children * school_score`
"""
...
def build_pipeline():
    ...
    log_stage("Validate OSRM", f"Connecting to local OSRM at {OSRM_URL}...")
    client = OSRMClient(OSRM_URL, ROUTING_CACHE_PATH)
    client.validate()
    log_stage("Validate OSRM Completed")
    ...
    log_stage("School Routing Analysis", "Computing and linking school accessibility routing via OSRM...")
    route_failures = 0
    route_failures += route_poi_links(
        raw_by_hex,
        schools,
        client,
        SCHOOL_PREFILTER_KM,
    ...
    log_stage("School Routing Analysis Completed", f"Failures: {route_failures}")
