...
EMIT_HTML_MAPS = os.environ.get("EMIT_HTML_MAPS", "0").strip().lower() in {"1", "true", "yes", "on"}
DATA_DIR = Path("DATA")
...
COORDINATE_AUDIT_PATH = AUDIT_DIR / f"{CITY_SLUG}_h3_coordinate_audit.json"
SPATIAL_ANALYSIS_AUDIT_PATH = AUDIT_DIR / f"{CITY_SLUG}_h3_spatial_analysis_audit.json"
...
KDE_RADIUS = 24
KDE_BLUR = 18
MAP_CENTER = json.loads(os.environ.get("CITY_MAP_CENTER_JSON", "[12.9716, 77.5946]"))

# A broad metro envelope. It keeps peripheral places that belong to the city
# while excluding obvious geocoding failures that would blow up the map extent.
...
def parse_localities(raw_localities):
    ...
    for idx, loc in enumerate(raw_localities):
        ...
        if not in_bounds(lat, lon, CITY_METRO_BOUNDS):
            outliers.append(
                {
                    "locality_id": locality_id,
                    "name": name,
                    "reason": "outside_city_metro_bounds",
                    "lat": lat,
                    "lon": lon,
                    "zone": zone_name,
                }
...
def main():
    ...
    coordinate_audit["html_maps_emitted"] = EMIT_HTML_MAPS
