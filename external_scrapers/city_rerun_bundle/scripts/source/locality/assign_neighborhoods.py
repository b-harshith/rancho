...
def main():
    ...
    localities_path = f"data/processed/99acres_{CITY_SLUG}_locality_coordinates.json"
    
    # Store in a city-specific folder using the configured slug
    output_dir = f"data/processed/{CITY_SLUG}_mapped_data"
    os.makedirs(output_dir, exist_ok=True)
    
    output_path = os.path.join(output_dir, f"{CITY_SLUG}_mapped_locality_boundaries.json")
    stats_path = os.path.join(output_dir, f"{CITY_SLUG}_neighborhood_stats.json")
    ...
    old_file_path = f"data/processed/99acres_{CITY_SLUG}_locality_with_neighborhoods.json"
