...
Output:
    data/raw/99acres_<city_slug>_localities.jsonl
"""
...
OUTPUT_FILE = OUTPUT_DIR / f"99acres_{CITY_SLUG}_localities.jsonl"
...
def capture_tokens_from_browser() -> dict:
    ...
    with sync_playwright() as p:
        ...
        # Scroll gradually so React/lazy-loaded sections actually render. A direct
        # jump to document.body.scrollHeight often skips the locality list.
        for attempt in range(30):
            ...
            # At the current bottom, pause for lazy loading and then try the
            # document bottom once more in case its height has increased.
            ...
            if button:
                ...
                log(
                    f"Browser: scrolling for 'Show More' "
                    f"(attempt {attempt+1}/30, y={after})..."
                )
        ...
        if not event.is_set():
            ...
            try:
                fallback_params = {**API_PARAMS, "page": 0}
