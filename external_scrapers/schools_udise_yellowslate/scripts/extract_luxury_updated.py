...
for s in data["schools"]:
    ...
    if fee_group == "Luxury":
        ...
        luxury_schools.append({
            ...
            "management": str(meta.get("management", "Unknown")).replace("|", ""),
            "students": enrollment.get("total_students", 0),
...
mgt_counts = defaultdict(int)
mgt_students = defaultdict(int)

for s in luxury_schools:
    mgt_counts[s["management"]] += 1
    mgt_students[s["management"]] += (s["students"] or 0)

md_lines = ["# Luxury Schools Overview\n\n"]
md_lines.append(f"**Total Luxury Schools identified:** {len(luxury_schools)}\n")
md_lines.append(f"**Total Students in Luxury Schools:** {sum(s['students'] or 0 for s in luxury_schools):,}\n\n")

md_lines.append("## Summary by Management Type\n")
md_lines.append("| Management Type | School Count | Total Students |")
md_lines.append("|---|---|---|")
for mgt in sorted(mgt_counts.keys(), key=lambda x: mgt_counts[x], reverse=True):
    md_lines.append(f"| {mgt} | {mgt_counts[mgt]:,} | {mgt_students[mgt]:,} |")

md_lines.append("\n## Detailed School List\n")
md_lines.append("| School Name | Management | Students | District | Block | Board | Confidence/Source |")
md_lines.append("|---|---|---|---|---|---|---|")

for s in luxury_schools:
    students_str = f"{s['students']:,}" if s['students'] else "N/A"
    md_lines.append(f"| {s['name']} | {s['management']} | {students_str} | {s['district']} | {s['block']} | {s['board']} | {s['confidence']} |")

with open("/Users/malleswararao/.gemini/antigravity-ide/brain/6fd3d189-67fb-4437-bda5-ffdd594340dc/luxury_schools.md", "w") as f:
    f.write("\n".join(md_lines))

print(f"Updated artifact with {len(luxury_schools)} schools, including management and student counts.")
