...
# Input Paths
UDISE_PATH = os.path.join(DATA_DIR, 'schools_analysis_delhi_ncr_compact.json')
AUTO_PATH = os.path.join(DATA_DIR, 'auto_matched_schools_delhi_ncr.json')
MANUAL_PATH = os.path.join(DATA_DIR, 'manual_matched_schools_delhi_ncr.json')
UNMATCHED_PATH = os.path.join(DATA_DIR, 'unmatched_yellowslate_delhi_ncr.json')

# Output Path
MASTER_OUTPUT_PATH = os.path.join(DATA_DIR, 'yellowslate_schools_master_delhi_ncr.json')
CACHE_PATH = os.path.join(DATA_DIR, 'nominatim_geocode_cache.json')

# Enrollment ratios computed from matched data
RATIOS_GRADES_2_9 = {
    "Pre-Primary": 0.0,
    "Primary / K-8": 0.8383,
...
def build_master():
    ...
    def get_cached_coords(school_name, area, pincode, city_source="delhi"):
        ...
        queries = []
        if pincode:
            queries.append(f"{school_name}, {pincode}, {city_suffix}")
        if area:
            queries.append(f"{school_name}, {area}, {city_suffix}")
        queries.append(f"{school_name}, {city_suffix}")
        if area:
            queries.append(f"{area}, {city_suffix}")
    ...
    def track_matched(ys_school, ud_school):
        ...
        bracket = fee_info.get("search_bracket_label") or "Unknown"
    ...
    for m in auto_matches:
        ...
        if lat is None or lon is None:
            lat, lon = get_cached_coords(name, ys.get("area", ""), ud.get("metadata", {}).get("pincode", ""), ys.get("city_source", "delhi"))
        ...
        master_list.append({
            ...
            "fee_bracket": {
                "bracket_key": fee_info.get("search_bracket_key", ""),
                "bracket_label": fee_info.get("search_bracket_label", ""),
    ...
    for m in manual_matches:
        ...
        lon = ud_loc.get("lng")
        if lat is None or lon is None:
            lat, lon = get_cached_coords(name, ys.get("area", ""), ud.get("metadata", {}).get("pincode", "") if ud else "", ys.get("city_source", "delhi"))
    ...
    for ys in unmatched_schools:
        ...
        lat, lon = get_cached_coords(name, ys.get("area", ""), "", ys.get("city_source", "delhi"))
    ...
    # Group by fee bracket and sort/rank
    grouped_by_bracket = {}
