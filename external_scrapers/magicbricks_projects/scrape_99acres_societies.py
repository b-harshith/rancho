#!/usr/bin/env python3
"""
99acres Bangalore Societies Scraper
======================================
...
Output:
    data/raw/99acres_bangalore_societies.jsonl
"""
...
TARGET_URL = "https://www.99acres.com/property-registry-records-in-bangalore-rrffid?entityType=PROJECT&referrer_section=CHECK_TRANSACTION_PRICES&src_page=REI_CITY_INSIGHTS&sortOrder=MOST_REGISTRY_RECORDS&timeDuration=1"
...
API_PARAMS = {
    ...
    "timeDuration":"1",
    "propType":    "1",
    "sortFilter":  "MOST_REGISTRY_RECORDS",
    "platform":    "MSITE",
    "pageType":    "RGR",
    "limit":       30,
}
...
OUTPUT_FILE = OUTPUT_DIR / "99acres_bangalore_societies.jsonl"

MAX_PAGES   = 100
SLEEP_SEC   = 0.8

# ── User's real session cookies (long-lived; update PROPLOGIN if it expires) ──
# Session material must be supplied at runtime; no embedded credentials are stored here.
...
def capture_tokens_from_browser() -> dict:
    ...
    with sync_playwright() as p:
        ...
        # Try scrolling + clicking Show More to trigger API call
        for attempt in range(12):
            ...
            if button:
                ...
                log(f"Browser: no 'Show More' yet (attempt {attempt+1}), waiting...")
        ...
        if not event.is_set():
            ...
            try:
                page.evaluate(f"""
                    fetch('{API_BASE}?cityId=20_LOCATION&entityType=PROJECT&budgetFilter=ANY&timeDuration=1&propType=1&sortFilter=MOST_REGISTRY_RECORDS&platform=MSITE&pageType=RGR&page=1&limit=30')
                """)
...
def run():
    ...
    while True:
        ...
        api_headers = {
            ...
            "pagename":           "RLP_RGR_CITY_PAGE", # changed to RLP_RGR_CITY_PAGE
            "platform":           "mobile",
            "referer":            TARGET_URL,
            "user-agent": (
                "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) "
        ...
        last_pg, count, expired = scrape_all_pages(api_headers)
        grand_total += count
