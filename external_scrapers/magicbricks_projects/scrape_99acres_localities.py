#!/usr/bin/env python3
"""
99acres Bangalore Localities Scraper
======================================
...
Then uses curl_cffi to scrape all ~1609 localities across ~54 pages.

Usage:
    python3 scrape_99acres_localities.py

Output:
    data/raw/99acres_bangalore_localities.jsonl
"""
...
OUTPUT_FILE = OUTPUT_DIR / "99acres_bangalore_localities.jsonl"

MAX_PAGES   = 60
SLEEP_SEC   = 0.8

# ── User's real session cookies (long-lived; update PROPLOGIN if it expires) ──
# Parsed from the network headers the user captured from their browser.
# PROPLOGIN expires ~2028, deviceid is static.
...
def capture_tokens_from_browser() -> dict:
    ...
    with sync_playwright() as p:
        ...
        if not event.is_set():
            ...
            try:
                page.evaluate(f"""
                    fetch('{API_BASE}?cityId=20_LOCATION&entityType=LOCALITY&budgetFilter=ANY&sortFilter=TOP_RATED&platform=MSITE&pageType=RNR&page=0&limit=30')
                """)
...
def scrape_all_pages(api_headers: dict) -> tuple:
    ...
    for pg in range(0, MAX_PAGES):
        ...
        log(f"  ✓ page={pg} | +{count} localities | total={total}")
...
def run():
    ...
    while True:
        ...
        if not expired:
            log(f"\n{'='*55}")
            log(f"ALL DONE!  Total localities scraped: {grand_total}")
